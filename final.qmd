...
# Introduction
In this task, I will be analyzing data from StackExchage History with the main focus on Vietnamese History. I will be going over some data preparing and cleaning process before analyzing. My analyzation are divided into 2 parts. A general and deep analyzation. A general analysis will be looking at the most common words and its trend. A deep analysis will be using some machine learning models and llms to understand the context of each posts and comments. 
...
I am interested in this column because it allows me to filter out tags related to `vietnam history`.
```{python}
...
print(post_string[:20])
print(comment_string[:20])
```


### Remove `HTML` tags from `Post` and `Comment` column
```{python}
print(re.search('<.*?>', post_string))
...
# Example display
print(re.search(link_pattern, comment_string))
print(re.search(bracket_pattern, comment_string))
...
#### Remove `punctuation`
```{python}
print(re.findall('[^\w\s]|[0-9]|[_]', post_string))
...
The map shows different events and locations from the `Post`. There are many interesting topics discussed and I have provided a summary for the majority of them on this Vietnam map. When you click on each of the marker, it will show the location and the event happened their. Also, the red line is the Ho Chi Minh Trail, the trail that sent North Vietnamese soilders and supplies into South Vietnam.
